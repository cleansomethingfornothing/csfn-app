    ...
    <page-transparent-header ref="page">
        <ion-page class="ion-page">
            ...
            <ion-content :scroll-events="true" class="user-content min-h-screen fullscreen"
                ...
                <ion-list class="pt-0 mb-20 list-large-items" lines="inset">
                    <!--
                ...
                <ion-list lines="inset">
                  <ion-list-header>
                    <ion-label class="font-bold text-xl">{{$t('activities-and-alerts')}}</ion-label>
                  </ion-list-header>
                  ...
                  <ion-item v-if="Object.keys(userCleanups).length === 0" class="w-full text-center p-2" lines="none">
                    <ion-label>{{$t('no-activities')}}</ion-label>
                  </ion-item>

                  <ion-item v-for="activity of userCleanups" :key="activity.id" button @click="openCleanup(activity.id)">
                    <ion-thumbnail slot="start">
                      <img alt="Cleanup image" class="w-full h-full object-cover object-center" :src="activity.pictures[0]">
                    </ion-thumbnail>
                    <ion-label>
                      <h2>
                        <b>{{$t(activity.done ? 'activity-in' : 'alert-in')}} {{activity.location.address.city}}</b>
                      </h2>
                      <p>
