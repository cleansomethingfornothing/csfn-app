        ...
        <ion-content>
            <ion-card class="top-0 w-full absolute z-40 m-0">
                <selection-list :elements="searchResults" element-key="id" icon="location"
                                :element-value="location => addressToString(location.address)" @selected="selected">
                </selection-list>
            </ion-card>
        ...
        <ion-footer mode="ios">
            <ion-toolbar v-if="showRadius">
                <ion-item>
                    ...
                    <ion-range min="0.5" max="5" color="primary" step="0.5"
                               @ionChange="radiusChange($event.target.value)"
