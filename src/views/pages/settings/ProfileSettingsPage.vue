<template>
  <ion-page>
    ...
    <ion-content>
      ...
      <ion-list>
        <input-item :label="$t('username')" :value="user.username" :readonly="true" ref="username-input"
                    @focus="doEditUsername">
          <div slot="end" class="m-auto">
            <ion-button fill="clear" shape="round" size="medium" color="medium"
                        style="width: 2.3rem; --padding-start: 2px; --padding-end: 2px">
              <ion-icon class="m-0" slot="icon-only" name="pencil-sharp"/>
            </ion-button>
          </div>
        </input-item>
        <ion-item>
          <ion-label position="stacked" class="font-bold">{{$t('country')}}</ion-label>
          <ion-select :value="user.country" @ionChange="updateUserCountry($event.detail.value)">
            <ion-select-option v-for="country in countries" :key="country.code" :value="country.code">{{country.emoji}}&nbsp;&nbsp;{{country.name}}</ion-select-option>
          </ion-select>
        </ion-item>
      </ion-list>
    </ion-content>
