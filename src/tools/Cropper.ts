export default class Cropper {
  ...
  public static cropSquare(image: Blob,
                           reduce?: boolean): Promise<Blob> {
  ...
  private isHorizontal(width,
                       height): boolean {
    return width > height
  }

  private processHorizontalImage(width,
                                 height) {
    const size = height
  ...
  private processVerticalImage(width,
                               height) {
    const size = width
