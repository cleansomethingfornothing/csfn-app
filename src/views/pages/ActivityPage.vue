            ...
            <transparent-header>
                <template slot="end-buttons">
                    <ion-button v-if="activity && activity.user.username === currentUser.username" fill="clear"
                                shape="round"
                                @click="$router.push('/edit?type=cleanup')">
                        ...
                        <div v-if="activity" class="ion-padding">
                            ...
                            <div class="my-1">
                                ...
                                <ion-chip v-if="activity.weight" color="secondary">
                                    <ion-icon :src="require('@/assets/img/icons/weight.svg')"
                            ...
                            <ion-item class="pl-0" lines="none">
                                <ion-icon class="mr-2" color="primary" name="location-sharp"></ion-icon>
                            ...
                            <div
                                ...
                                + `&markers=color:green|${activity.location.coords.lat},${activity.location.coords.lng}`
...
import MapModal from '@/views/pages/home/CleanupsMapPage.vue'
...
export default class ActivityPage extends Vue {
    ...
    mounted() {
        cleanupsModule.fetchCleanup(this.$route.params.id)
        setTimeout(() => {
    ...
    openPictures(selected) {
        ModalPresenter.present(this.$ionic, PicturesModal, {
            pictures: this.activity.pictures.map(({publicUrl}) => publicUrl),
            selected
        })
    }
